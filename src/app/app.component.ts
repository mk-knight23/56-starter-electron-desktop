...
export class App implements OnInit, OnDestroy {
  ...
  private get isElectron(): boolean {
    return isPlatformBrowser(this.platformId) && !!(window as any).electron;
  }
  ...
  ngOnInit() {
    if (isPlatformBrowser(this.platformId) && this.isElectron) {
      this.loadAppInfo();
    } else if (isPlatformBrowser(this.platformId)) {
      // Set default version for web mode
      this.appVersion.set('2.0.0 (Web)');
    }
  }
  ...
  ngOnDestroy() {
    ...
    if (isPlatformBrowser(this.platformId) && this.isElectron) {
      ...
      electron.removeListener('window-status', this.handleWindowStatus);
      electron.removeListener('menu-action', this.handleMenuAction);
      electron.removeListener('files-opened', this.handleFilesOpened);
    }
  }
  ...
  private initializeElectron() {
    ...
    electron.receive('window-status', (status: string) => {
      if (status === 'maximized') {
    ...
    electron.receive('menu-action', (action: string) => {
      console.log('Menu action:', action);
      // Handle menu actions here
    });

    // Listen for file openings
    electron.receive('files-opened', (files: string[]) => {
  ...
  private async loadAppInfo() {
    ...
    try {
      ...
      const platform = await electron.invoke('get-platform');
  ...
  onMinimize() {
    const electron = (window as any).electron;
    if (electron) {
      electron.send('window-control', 'minimize');
    }
  }
  ...
  onMaximize() {
    const electron = (window as any).electron;
    if (electron) {
      electron.send('window-control', 'maximize');
    }
  }
  ...
  onClose() {
    const electron = (window as any).electron;
    if (electron) {
      electron.send('window-control', 'close');
    }
  }
  ...
  async showNotification() {
    const electron = (window as any).electron;
    if (electron) {
      await electron.invoke('show-notification', {
        title: 'DesktopApp',
  ...
  async openFileDialog() {
    ...
    if (electron) {
      try {
        const result = await electron.invoke('show-message-box', {
          type: 'question',
          buttons: ['Yes', 'No'],
          defaultId: 0,
          title: 'Confirm',
      ...
      console.log('Dialog result:', result ? 0 : 1);
    }
  }
}
